...
class DikeNetwork:
    def __init__(self):
        ...
        # Load network
        G, dike_list, dike_branch, planning_steps = get_network(self.num_planning_steps)

        # Ensure DaysToThreat is initialized in the EWS node
        if "EWS" not in G.nodes:
            G.add_node("EWS")
        G.nodes["EWS"]["DaysToThreat"] = 0  # Set an appropriate initial value
        ...
        # Probability of exceedence for the discharge @ Lobith (i.e. times 6)
    ...
    def progressive_height_and_costs(self, G, dikenodes, steps):
        for dike in dikenodes:
            ...
            for s in steps:
                if f"DikeIncrease {s}" not in node:
                    node[f"DikeIncrease {s}"] = 0  # Initialize if not present
                node[f"DikeIncrease {s}"] *= self.dh
                # 1 Initialize fragility curve
                # 2 Shift it to the degree of dike heightening:
                ...
                # Calculate dike heightening costs:
